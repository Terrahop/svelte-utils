...
<script lang="ts">
  ...
  const getIncrement = (n: number): number => {
   ...
   */
  export const reset = (minimum = defaultMinimum): void => {
    width = minimum
    running = true
  }
   ...
   * @param minimum - Progress value to start at.
   */
  export const start = (minimum?: number): void => {
    reset(minimum)
    animate()
  }
   ...
   */
  export const complete = (settleTime = defaultSettleTime): void => {
   ...
   */
  export const setWidthRatio = (widthRatio: typeof width): void => {
    stop()
    width = widthRatio
    completed = false
    running = true
  ...
  beforeNavigate((nav) => {
    if (progressBarStartTimeout) {
      ...
      progressBarStartTimeout = undefined
    }
...
{#if running || width > 0}
  <output
    ...
    style="background-color: {color}; width: {width * 100}%; z-index: {zIndex};"
  >
    {#if running}
      <div
        class="svelte-progress-bar-leader"
        style="background-color: {color}; color: {color}; z-index: {zIndex + 1};"
      ></div>
    {/if}
  </output>
{/if}

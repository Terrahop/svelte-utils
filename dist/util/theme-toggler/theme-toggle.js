...
function toggleClasses() {
    ...
    for (const toggleElement of document.querySelectorAll('[data-toggle-theme]')) {
...
export const themeToggle = () => {
    ...
    for (const toggleElement of document.querySelectorAll('[data-toggle-theme]')) {
        ...
        function onClick() {
            ...
            if (!themesList)
                return;
            const themeJson = theme ? JSON.parse(theme) : toggleTheme0;
            if (themeJson === toggleTheme0) {
    ...
    return () => {
        for (const v of removeListeners)
            v();
    };
};
